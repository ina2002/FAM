...
\begin{remark}
需要说明的是，在实际群体决策中，专家所能提供的信息往往是不完全的：部分专家可能仅能给出自身权重，部分专家只能对部分指标进行排序，或仅能在某些指标下给出方案排序与相对重要性比值判断。对此，本文不要求所有输入信息在专家维、指标维和方案维上均完整给定，而是允许输入信息存在缺失。具体而言，若专家 $i$ 未对指标 $j$ 给出排序，则不生成相应的指标排序约束；若专家 $i$ 未在指标 $j$ 下对方案给出排序，则不生成对应的方案排序约束；若某组方案相对重要性比值信息未被提供，则不生成相应的比值约束。因而，后续模型中的各类约束索引集合仅由已观测到的有效输入构成，从而使模型能够兼容“部分可比、部分缺失、逐步补充”的专家信息输入情形。
\end{remark}
...
在上述问题描述基础上，本文进一步构建 FAM 的数学规划模型。模型的核心思想是：在满足给定专家权重、指标排序信息、方案排序信息以及方案相对重要性比值信息的前提下，首先确定各方案的综合权重及其排序结构；同时，考虑到实际决策中部分方案的综合表现可能非常接近，若仍强行赋予不同名次，可能夸大细微差异，因此模型进一步允许综合权重相同的方案处于同一排名层级。在此基础上，模型优先提高不同方案层级之间的区分度；随后，在不牺牲方案层排序效果的前提下，进一步提升各专家内部指标权重的差异程度，以避免指标权重塌缩为近似相等的结果。为此，本文采用两层优化思路：第一层目标关注方案层级排序结果的辨识性，第二层目标关注指标层权重结构的辨识性。
...
\subsection{第一层模型：允许同层的方案层级区分度最大化}

为提高最终排序结果的辨识性，同时允许综合表现非常接近的方案处于同一排名层级，设 $\tilde a_r$ 表示按综合权重排序后位于第 $r$ 位方案的权重，$d^{\mathcal A}$ 表示不同排名层级之间的最小区分度，并进一步引入二元变量
\begin{align}
    y_r=
    \begin{cases}
...
\end{align}
同时，设 $\lambda_r$ 为排序位置 $r$ 对应的差异调节参数，$\phi>0$ 为用于判定不同层级的最小分离阈值。于是，第一层目标仍写为
\begin{align}
    \max \ d^{\mathcal A}.
\end{align}

与严格线性排序不同，此时 $d^{\mathcal A}$ 不再刻画所有相邻排序位置之间的最小差距，而是仅刻画不同层级之间的最小差距。也就是说，当相邻两个排序位置被判定为同层时，模型允许其综合权重完全相等；当二者被判定为不同层级时，则要求其权重差异至少达到给定阈值 $\phi$，并进一步参与方案层区分度 $d^{\mathcal A}$ 的刻画。由此，模型既能够保证真正存在显著差异的方案被有效区分，又能够避免对综合表现极为接近的方案进行机械性的强行排序。
...
其中，
\begin{align}
    \Omega_i^C &:= \{(j,j')\mid \text{专家 } i \text{ 已给出 } j \text{ 与 } j' \text{ 的排序信息，且 } p_{ij}<p_{ij'}\}, \\
    \Omega_{ij}^A &:= \{(k,k')\mid \text{专家 } i \text{ 已在指标 } j \text{ 下给出 } k \text{ 与 } k' \text{ 的排序信息，且 } p_{ijk}<p_{ijk'}\}, 
\end{align}
...
\end{align}
其中 ,
\begin{align}\Omega^{R}   = \{(i,j,k,k')\mid \text{专家 } i \text{ 已在指标 } j \text{ 下给出方案 } k \text{ 与 } k' \text{ 的相对重要性比值}\}.
\end{align}
...
由于目标函数中涉及按综合权重排序后的方案位置权重 $\tilde a_r$，其本质上隐含了方案排序结构，难以直接写为标准数学规划形式。为此，进一步引入排序位置权重变量 $\tilde a_r$、$0$--$1$ 位置分配变量 $x_{kr}$，以及相邻位置层级判定变量 $y_r$，其中
\[
x_{kr}=\mathbf{1}(\text{方案 } k \text{ 位于第 } r \text{ 位}).
\]
这里，$\tilde a_r$ 表示排序后位于第 $r$ 位方案的综合权重。变量 $y_r$ 用于判定相邻两个排序位置是否属于同一层级：若 $y_r=0$，则要求 $\tilde a_r=\tilde a_{r+1}$；若 $y_r=1$，则要求二者至少相差 $\phi$，并由该差距参与方案层区分度 $d^{\mathcal A}$ 的刻画。

于是，第一层模型可改写为如下混合整数线性规划形式：
\begin{align}
    ...
    && \forall r=1,\dots,|\mathcal{K}|-1, \\
    & d^{\mathcal A}\le \lambda_{|\mathcal K|}\tilde a_{|\mathcal K|}, \\
    & u_{ij}-u_{ij'} \ge \varepsilon,
    && \forall i\in\mathcal{I},\ \forall (j,j')\in\Omega_i^C, \\
    & w_{ijk}-w_{ijk'} \ge \varepsilon,
    && \forall i\in\mathcal{I},\ \forall j\in\mathcal{J},\ \forall (k,k')\in\Omega_{ij}^A, \\
    ...
    && \forall r=1,\dots,|\mathcal{K}|-1.
\end{align}
其中，$d^{\mathcal A}$ 表示不同方案层级之间的最小区分度，$M$ 为足够大的正常数，$\phi$ 为判定两个相邻排序位置是否属于不同层级的最小分离阈值。约束
\begin{align*}
    \tilde a_r-\tilde a_{r+1} \ge \phi y_r,\qquad
    \tilde a_r-\tilde a_{r+1} \le M y_r
\end{align*}
共同刻画了层级判定逻辑：当 $y_r=0$ 时，有
\begin{align*}
...
\begin{align*}
    d^{\mathcal A}\le \lambda_r(\tilde a_r-\tilde a_{r+1}) + M(1-y_r)
\end{align*}
保证仅当第 $r$ 位与第 $r+1$ 位属于不同层级时，其差值才参与 $d^{\mathcal A}$ 的刻画；若二者属于同一层级，则该约束自动放松，不再要求这一对相邻位置为方案层区分度提供支撑。通过最大化 $d^{\mathcal A}$，模型优先提高不同方案层级之间的区分程度，同时允许综合表现极为接近的方案在最优解中呈现为相同权重。

\subsection{第二层模型：指标层差距最大化}

仅以上述第一层模型进行求解时，虽然能够保证排序一致性，并允许方案层出现合理的并列层级，但由于指标层仅受序关系约束，其权重差异仍可能退化为最小可行差距，从而出现“顺序存在但差距不足”的现象。为增强模型对专家内部指标重要性差异的识别能力，本文进一步引入如下区分度变量：
\begin{align}
    d_i^{\mathcal C}\ge 0,
    \qquad \forall i\in\mathcal{I},
\end{align}
...
在此基础上，为避免第二层优化放松第一层已经确立的严格排序关系，本文在第二层中同时保留指标层的最小容差约束与区分度约束。具体而言，对于任意由专家 $i$ 给出的有效指标排序对 $(j,j')\in\Omega_i^C$，一方面要求
\begin{align}
    u_{ij}-u_{ij'} \ge \varepsilon,
\end{align}
以保证排序靠前的指标权重始终严格大于排序靠后的指标权重；另一方面进一步要求
\begin{align}
    u_{ij}-u_{ij'} \ge d_i^{\mathcal C},
\end{align}
从而使 $d_i^{\mathcal C}$ 刻画专家 $i$ 内部不同指标之间可被进一步拉开的最小权重差异。由此，$\varepsilon$ 负责维持排序一致性的严格性，而 $d_i^{\mathcal C}$ 则用于度量并提升指标层权重结构的辨识性。显然，$d_i^{\mathcal C}$ 越大，说明专家 $i$ 对不同指标的重要性区分越明显。

为避免第二层优化破坏第一层已经获得的最优方案层级区分效果，本文采用词典序优化思想。设第一层模型的最优值为 $d^{\mathcal{A}\star}$，则第二层模型在附加约束
\begin{align}
...
\end{align}
该目标表示：在不牺牲方案层级区分度的前提下，进一步尽可能拉大各专家内部不同指标之间的权重差距。

因此，第二层模型可表述为
\begin{align}
    \max \quad & \sum_{i\in\mathcal I} d_i^{\mathcal C} \\
    \text{s.t.}\quad
    & d^{\mathcal A}=d^{\mathcal{A}\star}, \\
    & u_{ij}-u_{ij'} \ge \varepsilon,
    && \forall i\in\mathcal{I},\ \forall (j,j')\in\Omega_i^C, \\
...
由此，FAM 模型最终形成“先区分方案层级、再区分指标”的两层优化结构。第一层模型负责保证方案排序结果具有足够的辨识性，同时允许综合表现极为接近的方案处于同一层级；第二层模型则在保持第一层最优方案层级区分结果不变的前提下，进一步提升各专家内部指标权重的辨识能力。相较于原先同时内生识别专家权重的形式，该改进模型将专家权重外生化处理，从而使模型重点转向给定专家权重下的指标分解与方案层级排序问题，逻辑上更紧凑，也更便于与外部专家赋权方法相衔接。

\subsection{求解结果的聚合与输出}

在完成两层优化求解后，模型首先得到最小权重单元 $w_{ijk}^{\star}$、方案位置分配变量 $x_{kr}^{\star}$、排序位置权重变量 $\tilde a_r^{\star}$、层级判定变量 $y_r^{\star}$，以及区分度变量 $d^{\mathcal{A}\star}$ 和 $d_i^{\mathcal{C}\star}$。在此基础上，可进一步沿不同维度进行聚合并形成最终输出结果。
...
需要说明的是，在当前模型设定下，专家权重 $e_i$ 为外生给定参数，因此其不再作为模型求解输出，而是作为约束条件贯穿于最小权重单元的分配过程中。故模型最终输出主要包括：各专家对指标的局部权重贡献、各指标的全局权重、各方案的综合权重及其排序结果、方案层级划分结果，以及方案层和指标层的最小区分度指标。
